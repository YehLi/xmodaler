...
class GCNEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.gate_bias = nn.Parameter(torch.rand(relation_num, 1).cuda(), requires_grad=True)
        self.bias = nn.Parameter(torch.rand(relation_num, out_dim).cuda(), requires_grad=True)
    ...
    def forward(self, batched_inputs, mode=None):
        ...
        gcn_sub_gate = self.gcn_sub_gate_act(gcn_sub_gate.unsqueeze(2) + rel_gate_bias)
        gcn_sub = (gcn_sub.unsqueeze(2) + rel_bias)
        gcn_sub = (gcn_sub * gcn_sub_gate * rel_mask).sum(2)

        gcn_obj_gate = self.gcn_obj_gate_act(gcn_obj_gate.unsqueeze(1) + rel_gate_bias)
        gcn_obj = (gcn_obj.unsqueeze(1) + rel_bias)
